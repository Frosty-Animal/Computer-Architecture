-------------------------------------------------------------------------------------
              		Pipelined RISC-V Processor Changes
-------------------------------------------------------------------------------------
...
In the riscv module, we created a nested case statement for the store functions that determines what store instruction will be used and what part of the data will be loaded and stored.
...
Carry. These variables are all calculated in the alu module. This then determines whether PCSrcE is true or false. We also created a PCSrcNextE for jalr
...
In the datapath, there were some additions and changes:
  1: We created another 2-input mux that determines whether PCNextF will be PCNextTemp or 
  ALUResultE (This is for auipc)
  ...
  3. We converted resultmux from a mux3 to a mux4 to determine whether Result returns as 
  ALUResultW, ReadDataW, PCPlus4, or ResultLoad (which is a chunk of ReadData when calling a 
  specific load instruction)
  4. We added a mux2 for SrcAE that determines whether RegOutAE or PCE is returned.
  5. We called our module loadextend that handles what part of the ReadDataW should be loaded
  (This returns ResultLoad)
